...
echo "🚀 TaskMe Server Setup - Skip Nginx (Already Configured)"
echo "======================================================"
...
echo -e "${BLUE}📋 This script will setup TaskMe without configuring Nginx${NC}"
echo -e "${YELLOW}⚠️  Make sure you have already configured Nginx manually${NC}"
echo ""
...
echo "  - Not modifying existing Nginx configuration"
...
echo -e "${BLUE}🌐 Step 5: Checking Nginx configuration...${NC}"
if [ ! -f "/etc/nginx/sites-enabled/taskme" ]; then
    echo -e "${RED}❌ Nginx configuration for TaskMe not found!${NC}"
    echo -e "${YELLOW}Please add the following configuration to your Nginx:${NC}"
    echo ""
    echo -e "${BLUE}1. Create file: /etc/nginx/sites-available/taskme${NC}"
    echo -e "${BLUE}2. Add the configuration from TUTORIAL_SETUP_SKIP_NGINX.md${NC}"
    echo -e "${BLUE}3. Enable site: sudo ln -s /etc/nginx/sites-available/taskme /etc/nginx/sites-enabled/${NC}"
    echo -e "${BLUE}4. Test config: sudo nginx -t${NC}"
    echo -e "${BLUE}5. Reload nginx: sudo systemctl reload nginx${NC}"
    echo ""
    echo -e "${YELLOW}After configuring Nginx, run this script again.${NC}"
    exit 1
else
...
echo -e "  - No modification of existing Nginx configuration"
